...
class DitherEffectImpl extends Effect {
  ...
  set colorNum(value: number) {
    this.effectUniforms.get("colorNum")!.value = value;
  }

  get colorNum() {
    return this.effectUniforms.get("colorNum")!.value;
  }

  set pixelSize(value: number) {
    this.effectUniforms.get("pixelSize")!.value = value;
  }

  get pixelSize() {
    return this.effectUniforms.get("pixelSize")!.value;
  }
}
...
function DitherPlane() {
  ...
  useEffect(() => {
    const handlePointerMove = (event: PointerEvent) => {
      const dpr = gl.getPixelRatio();
      mouseRef.current.set(event.clientX * dpr, event.clientY * dpr);
    };
  ...
  useFrame(({ clock }) => {
    ...
    uniformsRef.current.mousePos.value.copy(mouseRef.current);
  });
